...
`Domain/StylistBrain.swift`'s `DecisionHierarchy` is the 7-tier priority order a stylist reasons in — hard constraints, dress code, weather, user profile, color harmony, fit/silhouette, aesthetic trend — and it's **lexicographic, not blended**: a lower-numbered tier always dominates a higher one, so an outfit that violates the dress code is a wrong answer regardless of how good its color harmony is. Each tier declares an `enforcement` (`.reject` for the validator's hard structural checks, `.penalize` for a scoring term, `.guide` for prompt-only guidance that may only break ties among otherwise-equal outfits — tier 7, aesthetic trend, is the only `.guide` tier).
