...
Routes 1, 2, and 4 are credit-gated via `creditGate`; 3 and 5 are uncapped beyond `rateLimitOnly`'s daily guardrail. **`RECOMMENDATION` cost is `0` (unmetered):** recommendations and wardrobe/Insights Q&A (both ride route 1) are the app's free core loop, so `creditGate("RECOMMENDATION")` always clears the balance check and debits nothing — it still runs (init/migration, usage-count increment, hard-cap check) and abuse is bounded by `rateLimitOnly`'s per-UID daily cap, not the wallet. Credits are therefore a pure **try-on render** (`IMAGE_GEN`) currency. `config/pricing` can re-meter recommendations at ops pace if that ever changes. `/openrouter/recommend` and `/openrouter/tryon` reuse the same `openrouterChatRouter` handler as `/openrouter/chat` — the separate mount points exist solely to attach per-operation `idempotencyGate`, `creditGate`, and `responseCache` middleware. `/openrouter/images` is the dedicated-Images-API branch the try-on and background-isolation services fall back to when `ModelConfig.isChatCompletionImageModel` is false.
