...
- **Every proxied network call also gets an `X-Idempotency-Key`** (a fresh UUID, also minted in `Services/ProxyAuthHeaders.swift`, same one-per-call cadence as `X-Request-Id` above) — required by `backend/functions/src/middleware/idempotency.ts`'s `idempotencyGate` on the three quota-gated OpenRouter routes (`docs/decisions/resolved-v1.md`'s "Idempotency Protection" entry), ignored elsewhere. If you add a call site that issues more than one distinct upstream attempt for one logical user action (like `OutfitRecommendationService`'s structured→unstructured fallback), make sure each attempt calls `ProxyAuthHeaders.current()` fresh rather than reusing headers across attempts — reusing the key would replay the first attempt's cached response instead of actually retrying.
