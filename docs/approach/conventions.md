...
- **No test or `#Preview` may require a real API key.** `Services/ServiceFactory.swift` exists specifically so tests/previews always get `Mock*` implementations by default; the real `OpenRouterIntentExtractionService`/`FalTryOnRenderService` are only reachable through that factory when a key is actually configured.
...
- **Long-running async work gets an explicit state enum**, not a bare `async throws` the caller has to guess a loading state around. `TryOnState` (`idle`/`submitting`/`polling(elapsedSeconds:)`/`succeeded`/`failed`) is the pattern to follow for any future multi-step async operation (e.g. a real ingestion pipeline).
