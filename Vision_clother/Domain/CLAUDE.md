...
- `StylistBrain.DecisionHierarchy` tiers are lexicographic, not blended: a lower tier (`.reject`/`.penalize` enforcement) always dominates a higher one; `.guide` tiers (aesthetic trend) may only break ties. Adding or reordering a tier renumbers every tier below it *and* the "Tier N" cross-references scattered through the prompt text — grep for `Tier ` and `tiers 1-` before and after.
- `ItemFeedbackChipCatalog` is a deterministic lookup table, never an LLM call — a tapped chip has no ambiguity to resolve. Taste chips write `ItemRating`, defect chips write `ItemNote`; never route a defect into an affinity map.
