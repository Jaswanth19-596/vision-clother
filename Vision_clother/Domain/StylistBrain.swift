...
enum StylistBrain {
    ...
    struct DynamicPromptComposer {
        ...
        static func composeSystemPrompt(
            ...
            OUTPUT FORMAT:
            - Every outfit must include top_id, bottom_id, and footwear_id whenever the wardrobe catalog above contains at least one item with that slot — always pick the best available match rather than leaving it null just because nothing is a perfect fit. Set one of these three to null ONLY if the catalog contains zero items for that slot at all (the user genuinely owns none) — in that case never invent, guess, or repurpose an id that isn't one of this catalog's own listed ids; leave it null and build the rest of the outfit around what they do own. outerwear_id, headwear_id, accessory_id, and bag_id are each optional — leave null unless the scenario/weather calls for them (see Tier 2 above for exactly when an accent slot is warranted).
            - The four optional keys above must always be present in the JSON output, but a present key is not a request to fill it: null is the correct and expected value for most outfits in most of these slots. Never treat "the key exists in the schema" as a reason to search the catalog for something plausible to put there — that is exactly how a bag ends up in an interview outfit. Decide per Tier 2 first; only populate the key if that decision says yes.
