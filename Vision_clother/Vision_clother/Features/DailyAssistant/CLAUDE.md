...
- **Wardrobe/Insights Q&A (2026-07-20):** a free-text turn that reads as a question (`Domain/QuestionIntentHeuristic.swift`) is tried against `StylistQAService` (a separate, smaller call — see `Services/CLAUDE.md`) before the ordinary recommendation flow. A confirmed answer renders as a plain `.answer` chat bubble, not the outfit carousel; anything else falls through to `resolveOutfits` unchanged.
