...
**Core Invariant (revised 2026-07-10):** The LLM is the primary outfit recommender. It receives the user's free-text prompt, a bounded catalog of wardrobe item *descriptions* (id, slot, formality, color/hex, undertone, pattern, seasonality, fabric weight, short description — never images at recommendation time), the user's derived style profile, weather, and color-theory guidance, and returns outfit picks that reference item IDs. A deterministic on-device validator (`Domain/OutfitRecommendationValidator.swift`) verifies every pick references a real, correctly-slotted, non-ghost item and re-scores survivors; it **falls back** to fully deterministic generation (`Domain/OutfitRecommendationEngine.swift`) when the LLM output is invalid, empty, or the API is unavailable. The wardrobe never leaves the device as images — only text/hex descriptions, and only for the bounded catalog. See `docs/decisions/resolved-v1.md` for the full rationale and the prior (superseded) invariant.
