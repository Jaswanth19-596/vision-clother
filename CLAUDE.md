...
* **Read Before Coding:** For non-trivial tasks (new features, bug fixes touching domain/service logic, architectural changes), read `timeline.md` at the start to understand chronological context, recent architectural changes, and schema evolution. Skip this for small, self-contained edits (typo fixes, copy tweaks, single-line changes) where the diff is self-explanatory — use judgment, and read it anyway if unsure whether prior context matters.
* **Update After Coding:** After implementing a feature or fixing a bug that changes behavior (not for trivial edits), update `timeline.md` with the date, status, problem statement, and precise file changes.

## 7. Documentation Sync — On Behavior-Changing Code Changes

Whenever you modify code in this repository in a way that changes documented behavior, you **must** check and update the documentation that describes it. Documentation drift (docs saying one thing while code does another) is treated as a bug. This does not apply to purely internal refactors, formatting, or changes with no externally-documented behavior — only check docs whose described behavior your specific change actually touches.
...
| Any change with user-visible or cross-file behavior impact | `timeline.md` (per Section 6 above) |
...
When context exceeds 120k tokens:
- Suggest /compact

Do not carry unrelated debugging history into new features.