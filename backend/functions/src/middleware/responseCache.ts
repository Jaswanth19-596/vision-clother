...
/**
 * Must run after verifyAuth (needs req.uid) and *before* quotaGate for
 ...
 * reaching quotaGate or the upstream OpenRouter fetch (zero quota charged,
...
export function responseCache(feature: string) {
  return async (req: AuthedRequest, res: Response, next: NextFunction): Promise<void> => {
    ...
    // Miss (or lookup failure) — let the request proceed to quotaGate/the
